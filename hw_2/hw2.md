...
5. terraform.tfvars - Значения переменных
6. key.json - Ключ сервисного аккаунта
...
Была назначена роль storage.admin для СА с помощью команды:
...
```
ssh -i ~/.ssh/id_rsa ubuntu@51.250.88.243
```

### Копирование данных

Данные скопированы из бакета s3://otus-mlops-ilnur-data/ в HDFS:

```
hadoop distcp s3a://otus-mlops-ilnur-data/ hdfs:///user/data/
```
...
![Мой скриншот](img/screenshot.png)

## 5. Оценка затрат на облачную инфраструктуру

Для оценки затрат использовался официальный тарифный калькулятор Yandex Cloud.

### Стоимость кластера Data Proc

**Итоговая стоимость в месяц (24/7):** **35 308,11 ₽**

> **Внимание:** Это расчёт для непрерывной работы кластера в течение всего месяца. В реальном проекте для экономии ресурсов кластер создаётся только на время вычислений и удаляется после завершения работы.

### Стоимость Object Storage

**Итоговая стоимость в месяц:** **8,87 ₽**

### Сравнение HDFS и Object Storage

| Хранилище | Объём | Стоимость в месяц | Разница |
|-----------|-------|-------------------|---------|
| **HDFS (Data Proc)** | 424 ГБ (зарезервировано) | **35 299,24 ₽** (в составе кластера) | — |
| **Object Storage** | 14.5 ГБ (фактически) | **8,87 ₽** | ~**4 000 раз дешевле** |

### Вывод

Использование HDFS в составе кластера Data Proc для долгосрочного хранения данных экономически нецелесообразно. Хранение данных в Object Storage обходится в **тысячи раз дешевле**, поэтому в архитектуре проекта данные хранятся в S3-совместимом хранилище (Object Storage), а кластер поднимается только на время вычислений и удаляется после завершения работы.

## 8. Удаление кластера

Для предотвращения лишних расходов кластер был полностью удалён с помощью Terraform:

```
cd terraform
terraform destroy

После удаления кластер перестал тарифицироваться. Данные в бакете Object Storage сохранены для дальнейшего использования.
```

```
(base) ilnurmustafin@Mac terraform % yc storage bucket list
+-----------------------+----------------------+--------------+-----------------------+---------------------+
|         NAME          |      FOLDER ID       |   MAX SIZE   | DEFAULT STORAGE CLASS |     CREATED AT      |
+-----------------------+----------------------+--------------+-----------------------+---------------------+
| otus-mlops-ilnur-data | b1g6v0u6dj5boh3nd6vl | 107374182400 | STANDARD              | 2026-08-15 07:33:51 |
+-----------------------+----------------------+--------------+-----------------------+---------------------+

(base) ilnurmustafin@Mac terraform % yc dataproc cluster list
+----+------+------------+--------+-------------+--------+
| ID | NAME | CREATED AT | HEALTH | ENVIRNOMENT | STATUS |
+----+------+------------+--------+-------------+--------+
+----+------+------------+--------+-------------+--------+

(base) ilnurmustafin@Mac terraform % yc compute instance list --folder-id b1g6v0u6dj5boh3nd6vl
+----+------+---------+--------+-------------+-------------+
